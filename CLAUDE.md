...
현재 등록된 프로젝트 키: `roguelike`, `babysanta`, `maeil`, `puzzle`
...
- Portfolio의 역기획 카테고리 카드(Slay the Spire, Stellar Blade)는 PDF 직접 링크(`href` + `target="_blank"`)이며, `data-project-btn`이 없으므로 모달이 열리지 않음
