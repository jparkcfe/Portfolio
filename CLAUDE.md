...
- **Portfolio** (`data-page="portfolio"`): 프로젝트 갤러리 (카테고리 필터 지원: 시스템기획/역기획/팀프로젝트)
...
현재 등록된 프로젝트 키: `roguelike`, `slaythespire`, `stellarblade`, `babysanta`, `maeil`, `puzzle`
...
```javascript
{
  ...
  overviewTitle: "게임 개요",  // 선택 (기본값: '게임 개요')
...
사용 가능한 카테고리: `all`, `시스템기획`, `역기획`, `팀프로젝트`
...
- `slide-puzzle-game.html`: 슬라이드 퍼즐 게임 (puzzle 프로젝트 모달에서 연결)
...
- Portfolio의 역기획 카드(Slay the Spire, Stellar Blade)는 모달 트리거(`data-project-btn` + `data-project`)를 사용하며, About 타임라인의 역기획 항목만 PDF 직접 링크(`href` + `target="_blank"`)로 동작
