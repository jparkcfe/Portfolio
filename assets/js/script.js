...
const projectData = {
  roguelike: {
    ...
    overview: "레벨업·상점·클리어 보상의 3경로 성장 시스템을 설계하고, 보상-비용 균형이라는 기획 목표를 50건의 플레이 테스트로 달성한 로그라이트 게임입니다.",
    concept: [
    ...
    systems: [
      ...
      {
        title: "보상 시스템",
        why: "즉각적 성장(레벨업)만으로는 전략적 깊이 부족, 장기 성장(골드)만으로는 성장 체감 부족",
        how: "웨이브 클리어 시 골드 보상(장기 목표)과 스탯 선택 보상(즉각 성장)을 동시 제공. 웨이브별 골드 보상량을 점진 증가로 설계하여 후반부 선택의 무게감 강화",
      ...
      {
        title: "보스 시스템",
        why: "일반 웨이브와 차별화된 긴장감, 보스 처치 시 성취감 제공",
        how: "돌진 패턴+십자 탄막 패턴 설계로 기존 몬스터와는 다른 패턴으로 난이도 상승",
        what: "단순 근접 패턴만 있을 때는 거리 유지+공격이 전부였으나, 돌진·탄막 도입 후 사전 동작을 보고 회피하는 능동적 플레이가 관찰됨"
      }
    ],
    videos: [
