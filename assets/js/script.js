...
const projectData = {
  roguelike: {
    ...
    overview: "레벨업·상점·클리어 보상의 3경로 성장 시스템을 설계하고, 50건의 플레이 테스트를 통해 보상-비용 균형을 검증한 로그라이트 게임입니다. AI 바이브 코딩(Claude)으로 타이틀부터 엔딩까지 6일 만에 1인 구현했습니다.",
    concept: [
  ...
  kanji: {
    title: "한자 낱말 퀴즈",
    tagline: "입력→판정→피드백, 가장 단순한 게임 루프를 설계하고 구현한 첫 프로젝트",
    ...
    overview: "기획→구현→검증 사이클의 첫 번째 프로젝트입니다. '입력→판정→피드백'이라는 가장 단순한 게임 루프를 설계하고, 이 구조가 이후 로그라이트의 카드 선택 시스템의 기반이 되었습니다.",
    concept: [
      "가장 단순한 게임 루프인 <strong>'입력→판정→피드백'</strong> 구조를 직접 설계·구현",
      "콤보 보너스 설계로 <strong>'연속 성공'이라는 단기 목표</strong>를 부여 — 단순 반복을 동기 부여 루프로 전환",
      "이 루프 구조가 이후 <strong>로그라이트의 카드 선택 시스템</strong> 설계 시 기반이 됨"
    ],
    systems: [
      {
        title: "퀴즈 루프 시스템",
  ...
  apple: {
    ...
    overview: "'합이 10이 되는 조합 찾기'라는 단순 규칙 위에, 2분 제한의 시간 압박과 콤보 보너스의 보상 설계를 얹은 프로젝트입니다. 규칙은 단순하되 긴장감과 동기 부여는 시스템으로 만드는 경험을 했습니다.",
    concept: [
  ...
  puzzle: {
    ...
    overview: "랜덤 셔플로 퍼즐을 생성하면 50% 확률로 '풀 수 없는 퍼즐'이 만들어지는 문제에 직면. 원인을 분석하고, 역생성 알고리즘(완성→역섞기)으로 구조적으로 해결한 프로젝트입니다. 이 '의도대로 작동하는지 검증'하는 습관이 이후 로그라이트 50건 테스트 기록으로 이어졌습니다.",
    concept: [
      "<strong>문제 발견</strong>: 랜덤 셔플 시 50% 확률로 해결 불가능한 퍼즐 생성",
      "<strong>구조적 해결</strong>: 완성 상태에서 역으로 섞는 역생성 알고리즘으로 100% 해결 가능 보장",
      "<strong>검증 습관 형성</strong>: '의도대로 작동하는가?' 확인하는 프로세스가 로그라이트 50건 밸런싱 테스트로 이어짐"
    ],
    systems: [
      {
        title: "역생성 알고리즘",
