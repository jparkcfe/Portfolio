...
const projectData = {
  roguelike: {
    ...
    tagline: "3경로 성장과 경제 밸런싱 — 보상과 비용의 기준을 세우고 조정한 로그라이트",
    ...
    overview: "3경로 성장 시스템을 설계하고, 6일간 3건의 밸런싱 케이스(경제 불균형·무기 차별화·레벨 디자인)를 Excel로 정리한 뒤 플레이 테스트 결과와 비교하며 수치를 조정한 로그라이트 게임입니다.",
    concept: [
      "<strong>3경로 성장 설계</strong>: 레벨업(저점 보장) / 상점(전략적 선택) / 클리어 보상(방향성 결정)으로 역할 분담",
      "<strong>밸런싱 Case 1 — 경제 불균형</strong>: 초기 총 보상 1,350G로 1스탯 MAX(11,333G) 불가. 비용 증가율 1.5배→1.35배, 기본 보상 50G→350G로 조정 (300G/+60G, 350G/+65G, 400G/+70G 세 값을 시도하여 비교). 10건 테스트를 바탕으로 보상-비용 밸런스를 조정",
      "<strong>밸런싱 Case 2 — 무기 차별화 부재</strong>: 초기 4종 무기가 데미지(15~20)만 달라 '다 똑같다'는 피드백을 받음. 데미지를 3~5로 하향하고 고유 부가효과(화상/슬로우/체인)를 추가하여 역할을 분리. 15건 테스트에서 선택 분포 삽30%/화염25%/얼음25%/번개20%로 편중 없음을 확인",
      "<strong>밸런싱 Case 3 — 레벨 디자인</strong>: 지수적 난이도 증가(Wave 9에서 256배)로 후반 플레이 불가. 선형 증가(+50%/wave, Wave 9에서 5배)로 전환한 뒤, 20건 테스트에서 Wave 9 도달률 15%→70%, Wave 10 클리어율 0%→40%로 개선 확인"
    ],
    target: {
    ...
    systems: [
      ...
      },
      {
        ...
        what: "1개 스탯 MAX 비용 5,457G ≈ 총 보상 5,490G로, 올인과 분산 사이의 선택 압박이 발생. 스탯 올인·무기 올인·균형 빌드를 테스트하며 Wave 10 보스 클리어 가능성을 확인"
      },
      {
        title: "보상 시스템 (리스크-리턴 수치 설계)",
        why: "초기 설계에서 총 보상 1,350G로 1스탯 MAX(11,333G)가 불가능했고, '부족한 스탯을 보상으로 채우자니 무기가 어중간'한 문제가 발생. 클리어 보상만으로 1개 스탯 MAX가 가능해야 성장 체감이 명확하다는 가설을 수립",
        ...
        what: "Wave 1-9 총 보상 5,490G vs 1스탯 MAX 5,457G를 기준으로 보상-비용 밸런스를 조정. 10건 테스트에서 9건 '성장이 체감된다' 피드백 확인"
      },
      {
        title: "특수 무기 시스템 (빌드 다양성 설계)",
        why: "초기 4종 무기가 데미지(15~20)만 달라 '다 똑같아서 아무거나 골라도 됨'이라는 테스트 피드백을 받음. 선택의 의미가 없어 빌드 다양성 목표에 실패",
      ...
      {
        title: "레벨 디자인 (난이도 곡선 조정)",
        why: "초기 적 스탯이 지수적 증가(2^(wave-1))하여 Wave 5에서 16배, Wave 9에서 256배에 도달. 플레이어 성장이 따라가지 못해 Wave 7 이후 사실상 플레이 불가",
        how: "선형 증가(+50%/wave)로 전환하여 Wave 5에서 3배, Wave 9에서 5배로 완화. 몬스터 3종(Wave 1/4/7 등장)으로 점진적 다양성 확보",
        what: "20건 테스트에서 Wave 9 도달률 15%→70%, Wave 10 클리어율 0%→40% 변화를 확인. 최종 평균 플레이타임 5.5분으로 목표(5~6분)에 근접"
      },
      {
        title: "보스 시스템 (패턴 설계)",
        ...
        what: "사전 동작을 보고 회피하는 능동적 플레이 패턴 관찰. Wave 10 클리어율 40%를 기준으로 적정 난이도 여부 확인"
      }
    ],
    videos: [
  ...
  slaythespire: {
    ...
    overview: "Slay the Spire 아이언클래드의 공격 카드 전체를 대상으로, 코스트 1당 기대 대미지를 기준선으로 설정한 뒤 모든 카드의 코스트 효율을 분석했습니다. 초과·미달 대미지의 비용-보정 구조를 도출하고, 부가 효과의 실질 대미지 기여를 수치 기준으로 검토하여 비용 구조를 5유형으로 분류했습니다.",
    concept: [
      "<strong>분석 프레임워크 구축</strong>: 코스트 1당 기대 대미지를 기준선으로 설정하고, 전체 공격 카드의 효율을 기준에 따라 비교",
      "<strong>비용 구조 5유형 분류</strong>: 초과 대미지형, 미달+보정형, 조건부 효율형, 시너지 의존형, 특수 메커니즘형으로 카드 비용 체계를 유형화",
      "<strong>불균형 발견과 개선안</strong>: 비용 유형 간 카드 수 불균형을 발견하고, 이를 보완하는 신규 카드안을 설계한 뒤 수치 기준으로 검토"
    ],
    systems: [
      {
        title: "코스트 효율 분석 프레임워크",
        why: "카드 밸런싱을 감각이 아닌 수치 기준으로 평가하기 위해, 모든 카드를 동일 척도로 비교할 수 있는 기준선이 필요",
        how: "코스트 1당 기대 대미지를 기준선으로 설정한 뒤, 각 카드의 실제 효율을 기준선 대비 %로 산출. 초과/미달 원인을 부가 효과·조건·시너지로 분해",
        what: "전체 공격 카드를 5가지 비용 유형으로 분류 완료. 유형별 설계 패턴과 밸런싱 의도를 문서화"
      },
      {
        title: "비용 유형 간 카드 수 불균형 분석 및 개선",
        why: "HP를 소모하는 카드군의 리스크-리턴 구조가 다른 유형 대비 불균형 — 리스크는 높지만 리턴이 비례하지 않는 카드가 다수 존재",
        how: "HP 소모 카드의 실질 효율을 산출하여 리스크 대비 리턴 비율을 수치화. 불균형 구간을 식별한 뒤, 해당 구간을 보완하는 신규 카드안을 설계",
        what: "신규 카드안의 코스트 효율이 기준선 내에 수렴하는지 수치 기준으로 검토. 분석에서 문제 발견, 설계, 검토까지의 과정을 문서화"
      }
    ],
    externalLinks: [
      {
  ...
  stellarblade: {
    ...
    concept: [
      "<strong>리스크-리턴 분석</strong>: 4종 방어 액션의 입력 타이밍 난이도(리스크)와 보상 크기(리턴)를 기준에 따라 비교하여 설계 의도 도출",
      "<strong>3작품 비교 분석</strong>: 세키로(체간 시스템), P의 거짓(리전 아츠), Stellar Blade(블링크/리펄스)의 방어 시스템 구조를 비교하여 각 게임의 설계 철학 차이 분석",
      "<strong>문제 발견과 개선안 제시</strong>: 다수전 피격 문제에 대한 슬로우 개선안, 스킬트리 학습 순서 왜곡에 대한 UI 개선안을 제안"
    ],
    systems: [
      {
        title: "방어 액션 리스크-리턴 분석",
        why: "4종 방어 액션이 각각 어떤 상황에서 최적인지, 플레이어에게 선택의 의미를 주는 구조인지 확인하기 위함",
        how: "각 액션의 입력 타이밍 허용 범위(리스크)와 성공 시 보상(리턴: 대미지, 무적 프레임, 에너지 회복)을 측정하여 리스크-리턴 매트릭스를 작성",
      ...
      },
      {
        ...
        how: "세키로의 체간/패링 중심 설계, P의 거짓의 리전 아츠/무기 분리 설계와 비교하여 Stellar Blade의 에너지 2종 분리가 빌드 다양성에 기여하는 구조를 분석",
        what: "다수전 피격 문제(피격 경직 중 추가 피격)에 대해 슬로우 연출 개선안을 제시. 스킬트리 학습 순서 왜곡(핵심 스킬이 후반에 배치)에 대해 UI 동선 개선안을 제시"
      }
    ],
    externalLinks: [
      {
  ...
  babysanta: {
    ...
    systems: [
      ...
      },
      {
        ...
        why: "맵 50% 이상 진행 후 추락하면 처음부터 재시작해야 하므로 캐주얼 유저 이탈 위험이 높음. 시작점으로 돌려보내는 것도 반복 피로를 유발하기 때문에, '가장 가까운 지점으로 즉시 복귀'가 피로감과 진행도 보존의 균형점",
  ...
  maeil: {
    ...
    info: {
      ...
      engine: "Unreal Engine 5",
      role: "시스템 기획 — VR 상호작용 5종 설계, 리소스 조율·기술 제약 대응 주도 (서브 PM 겸임)",
      team: "6인 (기획 2인 + 개발 3인 + 아트 1인)",
      period: "2025.03.17 — 2025.08.20",
      note: "NCA 장기과정 2기 쇼케이스 전시"
    },
    overview: "'보는 것 위주라 VR 장점이 없다'는 초기 피드백을 받고, 상호작용 5종을 기획·제안하여 프로젝트에 반영. '보는 VR'에서 '참여하는 VR'로 경험을 전환한 몰입형 VR 힐링 게임입니다.",
    concept: [
      "초기 피드백 '상호작용 부족'을 받고 <strong>VR 상호작용 5종을 기획·제안, 프로젝트에 반영</strong>",
    ...
    systems: [
      {
        ...
        how: "포인터로 목적지를 지정하면 즉시 이동. 2m 이내 근거리는 직접 걷기도 가능하게 하여 자연스러움 유지",
    ...
    teamwork: [
      {
        ...
        solution: "VR 레퍼런스를 분석한 뒤 상호작용 5개를 기획 및 제안 (버스카드 퍼즐, 창문 낙서, 버스 태그, 사진 업로드, 사진 감상)",
        result: "제안한 5개를 프로젝트에 반영, '보는 VR'에서 '참여하는 VR'로 전환"
      },
      {
        title: "기획-아트 리소스 조율",
        ...
        result: "맵 7개에서 5개로 축소, NPC 제거 합의. 최종 플레이 타임 8~9분"
      },
      {
        title: "기술 제약 대응",
        problem: "XR에서 VR로의 전환 연출이 Meta Quest에서 구현 불가. 기획팀이 '핵심 경험이 깨진다'고 주장하며 프로젝트 정체",
        solution: "기획 의도의 핵심을 정리하고 대안을 제시. 본질은 '변화를 직접 체험하는 감각'이므로, 2D에서 3D로의 전환 연출로 대체 가능하다고 설명",
        result: "팀 내 합의 도출, 2D에서 3D 전환 연출로 합의 및 구현 완료"
      }
    ],
    videos: [
